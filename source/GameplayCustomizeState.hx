...
class GameplayCustomizeState extends MusicBeatState
{
    ...
    public override function create() {
		...
		#else
		text = new FlxText(5, FlxG.height + 40, 0, "Tap and drag around gameplay elements to customize their positions. Press A to reset. X/Y to change zoom. Press B to go back.", 12);
		#end
		...
		#if android
		_pad = new FlxVirtualPad(NONE, A_B_X_Y);
		_pad.alpha = 0.65;
		add(_pad);
		#end
	...
	override function update(elapsed:Float) {
		...
		for (touch in FlxG.touches.list){
			if (touch.overlaps(sick) && touch.pressed)
			{
		...
		#else
		for (touch in FlxG.touches.list){
			if (touch.overlaps(sick) && touch.justReleased)
			{
